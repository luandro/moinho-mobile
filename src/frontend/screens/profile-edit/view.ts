...
export default function view(state$: Stream<State>, topBarElem$: Stream<ReactElement<any>>) {
	return xs.combine(state$, topBarElem$).map(([ state, topBarElem ]) => {
		const defaultName =
			!state.about.name || state.about.name === shortFeedId(state.about.id) ? '' : state.about.name;
			...
			h(
				...
				[
					h(View, { style: styles.avatarTouchable }, [
						h(Avatar, {
							size: avatarSize,
							url: state.newAvatar ? `file://${state.newAvatar}` : state.about.imageUrl,
							style: styles.avatar,
							overlayIcon: 'camera'
						})
					])
